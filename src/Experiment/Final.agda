...
module _ where
  ...
  record FunSem (repr : Type → Set) : Set where
    field lam : ∀ {s t} → (repr s → repr t) → repr (s ↦ t)
          _·_ : ∀ {s t} → repr (s ↦ t) → repr s → repr t 

  funF : Fragment
  funF .Semantics = FunSem 
...
module _ where
  ...
  term₃ : ∀[ Term [ arithF , funF ] nat ]
  term₃ = let double = (lam λ x → x + x) in double · (`n 2 * `n 3)

  term₄ : ∀[ Term [ arithF , boolF , funF ] nat ]
  term₄ = (if `b true then (lam λ n → n + n) · (`n 10) else `n 0) + `n 10
...
module _ where
  ...
  evalFun : Instance Eval funF
  FunSem.lam (inst evalFun) = λ f x → f x
  FunSem._·_ (inst evalFun) = λ f x → f x
  ...
  test₃ : ⟦ evalArith ∷ evalFun ∷ [] ⟧ term₃ ≡ 12
  test₃ = refl

  test₄ : ⟦ evalArith ∷ evalBool ∷ evalFun ∷ [] ⟧ term₄ ≡ 30
  test₄ = refl
